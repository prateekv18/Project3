...
public class MarksheetModelHibImp implements MarksheetModelInt {
	...
	public long add(MarksheetDTO dto) throws ApplicationException, DuplicateRecordException {
		...
		// get Student Name
		System.out.println("Model......." + dto.getStudentId());
		...
		System.out.println(
				"kkkkkk" + studentDTO.getFirstName() + " " + studentDTO.getLastName() + ",,,," + dto.getRollNo());
	 ...
	 * DuplicateRecordException { // TODO Auto-generated method stub Session
	 * session = null; Transaction tx = null;
	 * System.out.println("MArksheet Update run"); MarksheetDTO dtoExist =
	 * findByRollNo(dto.getRollNo()); System.out.println(dtoExist +
	 * "________________________"); // Check if updated Roll no already exist
	 * System.out.println("marksheetmodelHibImpl dtoexist.getRollNo= "+
	 * dtoExist.getRollNo()+"- - -");
	 * System.out.println("marksheetmodelHibImpl dto.getRollNo= "+
	 * dto.getRollNo()+"- - -");
	 * 
	 * // if (dtoExist != null && dtoExist.getId().equals(dto.getId())) { // //
	 ...
	 * 
	 * try { session = new
	 * Configuration().configure().buildSessionFactory().openSession(); tx =
	 * session.beginTransaction();
	 * System.out.println("update run"+dto.getPhysics()); session.merge(dto);
	 * System.out.println("update success"); tx.commit();
	 * 
	 * } catch (HibernateException e) { e.printStackTrace(); if (tx != null) {
	 * tx.rollback(); throw new
	 * ApplicationException("Exception in Marksheet Update" + e.getMessage()); }
	 * } finally { session.close(); } } Session session = null; Transaction tx =
	 * null; System.out.println("MArksheet Update run"); MarksheetDTO dtoExist =
	 * findByRollNo(dto.getRollNo()); System.out.println(dtoExist +
	 * "________________________"); // Check if updated Roll no already exist
	 * 
	 ...
	 * dto.setName(studentDTO.getFirstName() + " " + studentDTO.getLastName());
	 * 
	 * try { session = HibDataSource.getSession(); tx =
	 * session.beginTransaction(); System.out.println("update run");
	 * session.saveOrUpdate(dto); System.out.println("update success");
	 * tx.commit();
	 * 
	 * } catch (HibernateException e) {
	 * 
	 * if (tx != null) { tx.rollback(); throw new
	 * ApplicationException("Exception in Marksheet Update" + e.getMessage()); }
