...
public class MarksheetCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		System.out.println("kmkkkkkkkkkkk" + request.getParameter("studentId"));
		...
		if (DataUtility.getInt(request.getParameter("physics")) > 100
			...
			request.setAttribute("physics", "marks must be less than 100 and greater than or equal to 0");
			pass = false;
		}
		...
		if (DataUtility.getInt(request.getParameter("chemistry")) > 100
			...
			request.setAttribute("chemistry", "marks less than less than 100 and greater than or equal to 0");
			pass = false;
		}
		...
		if (DataUtility.getInt(request.getParameter("maths")) > 100
			...
			request.setAttribute("maths", "marks must be less than 100 and greater than or equal to 0");
			pass = false;
		}
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		System.out.println(request.getParameter("physics")+"<---physics");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SAVE.equalsIgnoreCase(op) || OP_UPDATE.equalsIgnoreCase(op)) {
			...
			try {
				if (id > 0) {
					System.out.println("marksheetctl update run");
					model.update(dto);
...
//		 MarksheetModelJDBCImpl model = new MarksheetModelJDBCImpl(); 
	        ...
	        if (OP_SAVE.equalsIgnoreCase(op)||OP_UPDATE.equalsIgnoreCase(op)) {
	            ...
	            MarksheetDTO dto = (MarksheetDTO) populateDTO(request);
	            System.out.println(dto.getPhysics()+"<-----physics in dopostt");
	            try {
	               
	            	if (id > 0) {
	            		System.out.println("if(id>0)"+"---- dto.getId()="+dto.getId());
	            		model.update(dto);
